...
- `stage` now aborts with a clear error if the branch contains a merge commit whose conflict resolution touches files outside `.github/workflows/` (detected via `git diff-tree --cc`). Those "evil merge" resolutions would otherwise be silently dropped when linearizing history for the public fork.
- `stage` also passes `--no-merges` when enumerating branch commits. Merge commits (typically used to pull `origin/<default>` back into a feature branch after a sync rewrite) can't be cherry-picked without `-m` and would abort staging. `--no-merges` still walks both sides of every merge, so non-merge content from either side is cherry-picked as normal and workflow commits from the merged-in side are filtered by `isWorkflowCommit`.
